...
Read the sequence beat to beat (per `storytelling-bible.md` → Beat definition). At each transition ask: does the audience have what they need to follow the logic jump to the next beat, or is there an assumed connective step that was never shown? A gap here is different from a deliberate ellipsis (skipping boring transit time is fine) — the test is whether the CAUSAL logic survives the cut, not whether every second is shown.

## 2. Continuity Gap Detector
Cross-check adjacent shots against `continuity-bible.md`: does shot state actually carry forward (wardrobe, props, position, lighting, time) or is there an unexplained jump? This is the shot-to-shot version of §1's scene-to-scene check.
...
- **Identity drift**: not just face — wardrobe, body proportion, voice, personality consistent with the locked cast (`workflows/casting.md` → Character Lock)?
...
Drift is not always wrong — a deliberate visual arc (e.g. increasing chaos as a character unravels) is intentional evolution, not drift. The test is whether the change was a decision or an accident; an accidental drift has no stated reason in `decisions.log` (`project-state-system.md` → Decision Provenance), a deliberate arc does.
